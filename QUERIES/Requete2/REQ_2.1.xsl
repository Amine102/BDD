                ...
                <xsl:for-each-group select="dataset/ligne[date/@annee = '2016']" group-by="localisation/ville">                    
                        ...
                        <xsl:for-each-group select="current-group()" group-by="categorie/@nom">
                            <tr>
                                ...
                                <xsl:variable name="homme_total" select="avg(current-group()[genre/@nom = 'Homme']/salaire)" />
                                <xsl:variable name="femme_total" select="avg(current-group()[genre/@nom = 'Femme']/salaire)" />
                                <td><xsl:value-of select="format-number(((($homme_total - $femme_total) div $femme_total) * 100), '#.##')" /></td>
                            </tr>
                        </xsl:for-each-group>                    
                </xsl:for-each-group>
            </tbody>
